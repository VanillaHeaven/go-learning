...
func (s *luaState) Arith(aop ArithOP) {
	var a, b luaValue
	b = s.stack.pop()
	if aop != LUA_OPUNM && aop != LUA_OPBNOT {
		a = s.stack.pop()
	} else {
		a = b
	}
	...
	op := operators[aop]
	if result := _arith(a, b, op); result != nil {
		s.stack.push(result)
	} else {
...
func _arith(a, b luaValue, op operator) luaValue {
	if op.floatFunc == nil {
		if x, ok := convertInteger(a); ok {
			if y, ok := convertInteger(b); ok {
